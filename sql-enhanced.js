...
Draw.loadPlugin(function(ui) {
	...
	// Extends Extras menu
	mxResources.parse('fromSqlEnhanced=From SQL Enhanced');

	var wnd = new mxWindow(mxResources.get('fromSqlEnhanced'), div, document.body.offsetWidth - 480, 140,
		320, 300, true, true);
	...
	function CreateTableUI() {
		...
		tableList.forEach(function(tableModel) {
			...
			var maxNameLength = 100 + tableModel.Name.length;

			var headerProps=[];
			headerProps.push("swimlane");
			headerProps.push("fontStyle=0");
			...
			headerProps.push("collapsible=1");
			headerProps.push("marginBottom=0");
			headerProps.push("swimlaneFillColor=#ffffff");
			headerProps.push("align=center");

			headerPropList=headerProps.join(";");


			//Create Table
			tableCell = new mxCell(tableModel.Name, new mxGeometry(dx, 0, maxNameLength, 26),
				headerPropList);
			...
			var size = ui.editor.graph.getPreferredSizeForCell(rowCell);
			if (size !== null) {
				tableCell.geometry.width = size.width + maxNameLength;
			}
	...
	// Adds action
	ui.actions.addAction('fromSqlEnhanced', function() {
	...
	theMenu.funct = function(menu, parent) {
		...
		ui.menus.addMenuItems(menu, ['fromSqlEnhanced'], parent);
	};
});